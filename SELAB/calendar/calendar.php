...
$sql = "SELECT * FROM attendance_check where id=$user_id";
$result = mysqli_query($dbconn, $sql);
while ($R = mysqli_fetch_array($result)) {
    $attend[] = array(0 => date("n-j", strtotime($R['attendance_day'])), 1 => $R['id'], 2=>$R['attendance']);
}
?>
...
<?php
    ...
    while ($date <= $last_day) {
        ...
        for ($i = 0; $i < count($attend); $i++) {
            if ($attend[$i][0] == "$month-$date" && $attend[$i][2] == 1) {
                $class = "attend";
            }
        }
